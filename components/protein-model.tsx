...
import { Tube, Line, Instances, Instance, MeshTransmissionMaterial } from "@react-three/drei"
import { Vector3, CatmullRomCurve3, Color, DoubleSide } from "three"
import { useFrame } from "@react-three/fiber"
import { getColorByScheme } from "../utils/protein-colors"

// Create a custom helix geometry
function Helix({ start, end, radius, color, turns, segments = 30 }) {
...
// Create a beta sheet geometry
function BetaSheet({ points, width = 3, color }) {
  const curve = useMemo(() => new CatmullRomCurve3(points.map((p) => new Vector3(p[0], p[1], p[2]))), [points])

  return (
    <Tube args={[curve, 64, width, 8, false]}>
      <meshStandardMaterial color={color} roughness={0.4} metalness={0.3} side={DoubleSide} flatShading />
    </Tube>
  )
}
...
export function ProteinModel({
  ...
  const secondaryStructures = useMemo(() => {
    if (!protein || !protein.chains || visualizationMode !== "cartoon") return []
    ...
    protein.chains.forEach((chain) => {
      ...
      let currentStructure = { type: null, residues: [] }

      chain.residues.forEach((residue, index) => {
        if (residue.ss !== currentStructure.type && currentStructure.residues.length > 0) {
  ...
  const bonds = useMemo(() => {
    if (!protein || !protein.chains || visualizationMode !== "ball-and-stick") return []
      ...
      chain.residues.forEach((residue) => {
        ...
        if (residue.bonds) {
          residue.bonds.forEach((bond) => {
            ...
            // Skip hydrogens if not showing them
            if (!showHydrogens && (bond[0].startsWith("H") || bond[1].startsWith("H"))) return
      ...
      {visualizationMode === "cartoon" && (
        <>
          {secondaryStructures.map((structure, index) => {
            const color = getColorByScheme({ chainId: structure.chainId, ss: structure.type }, colorScheme)

            if (structure.type === "H") {
              // Alpha Helix
              const startResidue = structure.residues[0]
              const endResidue = structure.residues[structure.residues.length - 1]

              if (!startResidue.atoms.CA || !endResidue.atoms.CA) return null

              const start = [startResidue.atoms.CA.x, startResidue.atoms.CA.y, startResidue.atoms.CA.z]
              const end = [endResidue.atoms.CA.x, endResidue.atoms.CA.y, endResidue.atoms.CA.z]
              ...
              return <BetaSheet key={`sheet-${index}`} points={points} width={2.5} color={color} />
            } else {
              // Loop/Coil
              const points = structure.residues
                .filter((r) => r.atoms.CA)
                .map((r) => new Vector3(r.atoms.CA.x, r.atoms.CA.y, r.atoms.CA.z))
      ...
      {visualizationMode === "ball-and-stick" && (
        <>
          ...
          {bonds.map((bond, index) => (
            <Line key={`bond-${index}`} points={[bond.start, bond.end]} color={new Color("#cccccc")} lineWidth={2} />
          ))}
        </>
      )}
